...
os.system("#!/usr/bin/env python3")
os.system("chmod a+x ask.py")
...
def print_qa(qa_list, show_answers=False):
...
if __name__ == "__main__":
    # Files
    #Article = sys.argv[1] # article.txt
    #N_questions = int(sys.argv[2]) # nquestions
    
    Article = 'Development_data/set4/a10.txt' # article.txt
    N_questions = 4 # nquestions
                   
                            
                
                
    qg = QuestionGenerator()
    path_context = Article
    
    with open(path_context, 'r') as a:
        context = a.read()
    ...
    out_file = open("Ask_Output.txt", "w")
    for qa in qa_list:
        print(qa['question'])
        out_file.write("%s \n" % qa['question'])
        sys.stdout.write("%s \n" % qa['question'])
